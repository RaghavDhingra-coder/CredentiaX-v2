...
// Lazy-load QRScanner so camera libs don't bloat the initial bundle
...
export default function VerifierDashboard() {
  ...
  async function verifyCertId(id) {
    ...
    setResult(null)
    try {
      const { data } = await api.get(`/certificates/${trimmed}`)
      setResult({ found: true, cert: data.data.certificate })
    } catch (err) {
      if (err.response?.status === 404) {
        setResult({ found: false })
      } else {
        toast.error('Verification request failed. Try again.')
      }
    } finally {
      setChecking(false)
    }
  ...
  // Called by QRScanner when it successfully reads a valid cert ID
  function onQRScan(id) {
    setShowScanner(false)
    toast.success(`QR detected: ${id}`)
    verifyCertId(id)
  }
  ...
  return (
    <div className="w-full">
          ...
          <div className="lg:col-span-2 flex flex-col gap-5">
            ...
            <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-5 sm:p-6">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="font-semibold text-white text-sm sm:text-base">Scan Certificate QR</h2>
                  <p className="text-slate-500 text-xs mt-0.5">Open camera and point at any CredentiaX QR code</p>
                </div>
            ...
            {/* Manual search */}
            <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-5 sm:p-6">
              <h2 className="font-semibold text-white text-sm sm:text-base mb-4">Verify by Certificate ID</h2>
              <form onSubmit={onVerify} className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-3 rounded-xl bg-slate-800 border border-slate-600 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                  />
                </div>
                <button
                  type="submit"
                  disabled={checking || !certId.trim()}
                  className="px-4 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold transition-all whitespace-nowrap"
                >
              ...
              {checking && (
                <div className="flex items-center gap-2 mt-4">
                  ...
                  <span className="text-slate-400 text-sm">Checking certificate registry…</span>
                </div>
              )}

              {result && <ResultCard result={result} onReset={() => { setResult(null); setCertId('') }} />}
            </div>
          ...
          <div className="flex flex-col gap-4 sm:gap-5">
            ...
            <div className="bg-slate-900 border border-slate-700/60 rounded-2xl p-5 sm:p-6">
              ...
              <ol className="flex flex-col gap-2.5">
                {[
                  'Click "Open QR Scanner" and allow camera access',
                  'Point camera at the QR code on the certificate',
                  'Certificate is verified automatically — no typing needed',
                  'Or type a Certificate ID manually and click Verify',
                ].map((step, i) => (
      ...
      {/* QR Scanner modal — lazy loaded, only mounted when open */}
      {showScanner && (
